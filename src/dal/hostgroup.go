...
func ReadNagiosHostGroupFromFileTask(file string, outputChannel chan *model.HostGroup, waitG *sync.WaitGroup) {
	if utils.IsFile(file) {
		...
		for _, line := range linesOfText {
			...
			if reEndDefineStatement.MatchString(line) && strings.Compare(define, "") > 0 {
				define += "\n"
				define += line
				outputChannel <- model.NewNagiosHostGroup(define)
				define = ""
				continue
			}

			if reStartHostGroup.MatchString(line) {
				define = line
